  ...
  do k = 1, nz
    do j = 1, ny
      do i = 1, nx
        ...
        if ((w_lorentz_p(i,j,k).gt.1.5d0) .or. (w_lorentz_p(i,j,k).lt.1.0d0)) then
          W_guess = 1.0d0
          EOS_rho(1) = dens(i,j,k)/sdet
          EOS_T(1) = 1.0d0
          EOS_ye(1) = Y_e_con(i,j,k)/dens(i,j,k)
          call EOS_Omni_press(GRHydro_eos_handle,EOS_Tflag,GRHydro_eos_rf_prec,EOS_n,&
            EOS_rho,EOS_eps,EOS_T,EOS_ye,EOS_press,EOS_keyerr,EOS_anyerr)
          z_guess = ((EOS_rho(1)*(1.0d0 + EOS_eps(1)) + EOS_press(1)))
          T_guess = 1.0d0
        else
...
subroutine Con2Prim_3DNR_1DD_hot(cctk_iteration, &
  ...
  if(success_3DNR .eq. .false.) then
    !$OMP CRITICAL
    write(warnline,*) "3DNR failed, using Dekker."
    call CCTK_WARN(1,warnline)
    !$OMP END CRITICAL
...
subroutine Con2Prim_3DNR_hot(W_guess, z_guess, T_guess, udens, usx, usy, usz, s2, utau, uye_con, success, &
  ...
  do while(done .eq. .false.)
    ...
    call Con2Prim_3DNR_hot_cons2(W_current,z_current,s2,cons2)
    call Con2Prim_3DNR_hot_cons3(W_current,z_current,udens,press_current,eps_current,cons3)
...
subroutine Con2Prim_DekkerInvertTemp(eps_target, T_guess, rho, Ye, result, handle)
  ...
  temp_inversion_reldiff = 1.0d-9
  temp_inversion_absdiff = 1.0d-10
